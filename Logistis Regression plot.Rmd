...
plot_lr_cv <- function(df,start_id, end_id, bar_color, save_name){
  df %>% 
  ggplot(aes(x = id, y = mean)) +
  geom_bar(aes(x = id, y = mean), stat="identity", fill=bar_color, alpha=0.7) +
  geom_errorbar(aes(x = id, ymin = mean-std_err, ymax = mean+std_err), width=0.25, colour="black", alpha=0.9, size=1.3) +
  ...
  #ggsave(save_name, units = "px")
}
  
```

```{r}
gray <- collect_lr_cv("./exp data/gray_BC_SAR/Gray Person")

plot_lr_cv(gray, 21, 40, "dodgerblue4", "LR_gray.png")
```


```{r}
color <- collect_lr_cv("./exp data/color_BC_SAR/Color Person")

plot_lr_cv(color, 1, 20, "red3", "LR_color.png")
...
color_folder <- "./exp data/color_BC_SAR/Color Person"
gray_folder <- "./exp data/gray_BC_SAR/Gray Person"
...
p_plot <- both_cond %>% 
  ...
  geom_errorbar(aes(x = attribute, ymin = median-diff_min_med, ymax = median+diff_max_med), position = position_dodge(width = 0.9), width=0.25, colour="black", alpha=0.9, size=1.3) +
  xlab("attribute") +
  ylab("count") +
  ggtitle("Prediction of Binary classification label using semantic ratings") +
  theme_bw() +
