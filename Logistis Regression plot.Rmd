...
collect_lr_cv <- function(folder){
  ...
  for (id in 1:20) {
      #filename <- str_c("gray_people/Gray Person",id, ".csv")
      filename <- str_c(folder,id, ".csv")
      cv_res <- LR_cv(filename,  seed_num=5)
...
```{r}
gray <- collect_lr_cv("gray_people/Gray Person")
...
```{r}
color <- collect_lr_cv("color_people/Person")
...
```
```{r}
LR_cv("gray_people/Gray Person1.csv",  seed_num=5)
```
...
p1 <- collect_p("gray_people/Gray Person15.csv",  seed_num=25)
p1
```
...
#p_f1 <- collect_one_fold("color_people/Person",5)
#p_f1


color_folder <- "color_people/Person"
gray_folder <- "gray_people/Gray Person"
...
#sum(p_f1$see_throughness <0.05, na.rm=TRUE)
...
both_cond %>% 
  ggplot(aes(x = attribute, y = median, fill = factor(condition))) +
